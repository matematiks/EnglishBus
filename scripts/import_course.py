...
def import_course(course_folder_path, course_name, force=False):
    ...
    for i, row in df.iterrows():
        ...
        # Match Assets (Convention: 001.jpg, ing_001.mp3, tr_001.mp3)
        # OR based on ID column if exists, otherwise row index + 1
        file_id = row.get('id', i + 1)
        
        # Formats to check
        img_candidates = [f"{file_id:03}.webp", f"{file_id:03}.jpg", f"{file_id:03}.jpeg", f"{file_id:03}.png", f"{file_id}.jpg"]
        audio_en_candidates = [f"ing_{file_id:03}.mp3", f"en_{file_id:03}.mp3", f"{file_id:03}.mp3"]
        audio_tr_candidates = [f"tr_{file_id:03}.mp3"]
        
        # Find paths (relative for URL)
        image_url = None
