...
@router.get("/courses/{course_id}/stats/repetition")
def get_course_repetition_stats(
    course_id: int,
    user_id: int,
    db: sqlite3.Connection = Depends(get_db)
):
    """
    Get repetition breakdown for statistics cards.
    Categories (Adjusted for Motivation):
    - New Seen: Repetition 1-2
    - Mid Level: Repetition 3-6
    - Mastered: Repetition 7+
    """
    try:
        # Get all words in course for this user
        cursor = db.execute("""
            SELECT repetition_count
            FROM UserProgress P
            JOIN Words W ON P.word_id = W.id
            WHERE P.user_id = ? AND W.course_id = ? AND P.repetition_count > 0
        """, (user_id, course_id))
        ...
        return {"new_seen": 0, "mid_level": 0, "mastered": 0}
